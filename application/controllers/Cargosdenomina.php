...
$resInsert = $this->pedeo->insertRow('INSERT INTO napn(apn_name, apn_riskclass, apn_status) VALUES(:apn_name, :apn_riskclass, :apn_status)', array(
...
$resUpdate = $this->pedeo->updateRow('UPDATE napn SET apn_name = :apn_name , apn_riskclass = :apn_riskclass , apn_status = :apn_status  WHERE apn_id = :apn_id', array(
'apn_name' => $Data['apn_name'], 
'apn_riskclass' => $Data['apn_riskclass'], 
'apn_status' => $Data['apn_status'], 
'apn_id' => $Data['apn_id']));
...
$resSelect = $this->pedeo->queryTable("SELECT apn_name , narl.arl_profit AS apn_riskclass, apn_id , CASE  WHEN apn_status::numeric = 1 THEN 'Activo' WHEN apn_status::numeric = 0 THEN 'Inactivo' END AS apn_status FROM napn LEFT JOIN narl ON napn.apn_riskclass::numeric = narl.arl_id ",array());
