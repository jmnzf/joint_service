...
class SalesNd extends REST_Controller
{
			...
			if (is_numeric($resInsert) && $resInsert > 0) {
				...
				foreach ($ContenidoDetalle as $key => $detail) {
					...
					$sqlInsertDetail = "INSERT INTO vnd1(nd1_docentry, nd1_itemcode, nd1_itemname, nd1_quantity, nd1_uom, nd1_whscode,
										...
										detalle_anuncio,imponible,nd1_clean_quantity,nd1_acciva,nd1_codimp,nd1_vat_ad,nd1_vatsum_ad,nd1_accimp_ad,nd1_codimp_ad, nd1_codmunicipality)
										...
										:nd1_clean_quantity,:nd1_acciva,:nd1_codimp,:nd1_vat_ad,:nd1_vatsum_ad,:nd1_accimp_ad,:nd1_codimp_ad, :nd1_codmunicipality)";
							...
							if (intval(count($ContenidoRentencion)) > 0) {
								...
								foreach ($ContenidoRentencion as $key => $value) {
									...
									$resInsertRetenciones = $this->pedeo->insertRow($sqlInsertRetenciones, array(
										...
										':crt_linenum'   => $detail['nd1_linenum'],
						...
						if (intval(count($DetalleConsolidadoRetencion)) > 0) {
							...
							if (isset($Data['vnd_totalretiva']) && is_numeric($Data['vnd_totalretiva']) && ( $Data['vnd_totalretiva'] * -1  > 0 ) ){
								...
								if (trim($Data['vnd_currency']) != $MONEDALOCAL) {
									...
									$RetencionDescuentoLOC = $RetencionDescuentoLOC + ( ( $Data['vnd_totalretiva'] * -1 ) * $TasaDocLoc);
								
								}else{
									$RetencionDescuentoLOC = $Data['vnd_totalretiva'] * -1;
								}
								...
								if (trim($Data['vnd_currency']) != $MONEDASYS) {
									...
									$RetencionDescuentoSYS = $RetencionDescuentoSYS + ( ( $Data['vnd_totalretiva'] * -1 ) / $TasaLocSys );
								} else {
		
									$RetencionDescuentoSYS = ( $Data['vnd_totalretiva'] * -1 );
								}
							...
							if (isset($Data['vnd_totalret']) && is_numeric($Data['vnd_totalret']) && ( $Data['vnd_totalret'] * -1  > 0 ) ){

								if (trim($Data['vnd_currency']) != $MONEDALOCAL) {

									$RetencionDescuentoLOC = $RetencionDescuentoLOC + ( ( $Data['vnd_totalret'] * -1 ) * $TasaDocLoc);
								
								}else{
									$RetencionDescuentoLOC = $RetencionDescuentoLOC + ($Data['vnd_totalret'] * -1);
								}


								if (trim($Data['vnd_currency']) != $MONEDASYS) {

									$RetencionDescuentoSYS = $RetencionDescuentoSYS + ( ( $Data['vnd_totalret'] * -1 ) / $TasaLocSys );
								} else {
		
									$RetencionDescuentoSYS =  $Data['vnd_totalret'] * -1 ;
								}
							}
	...
	public function updateSalesNd_post()
	{
		...
		if (is_numeric($resUpdate) && $resUpdate == 1) {
			...
			foreach ($ContenidoDetalle as $key => $detail) {
				...
				$sqlInsertDetail = "INSERT INTO vnd1(nd1_docentry, nd1_itemcode, nd1_itemname, nd1_quantity, nd1_uom, nd1_whscode,
							nd1_price, nd1_vat, nd1_vatsum, nd1_discount, nd1_linetotal, nd1_costcode, nd1_ubusiness, nd1_project,
							nd1_acctcode, nd1_basetype, nd1_doctype, nd1_avprice, nd1_inventory, nd1_acciva,nd1_ubication,nd1_clean_quantity, nd1_codmunicipality)VALUES(:nd1_docentry, :nd1_itemcode, :nd1_itemname, :nd1_quantity,
							:nd1_uom, :nd1_whscode,:nd1_price, :nd1_vat, :nd1_vatsum, :nd1_discount, :nd1_linetotal, :nd1_costcode, :nd1_ubusiness, :nd1_project,
							:nd1_acctcode, :nd1_basetype, :nd1_doctype, :nd1_avprice, :nd1_inventory, :nd1_acciva,:nd1_ubication,:nd1_clean_quantity, :nd1_codmunicipality)";
	...
	public function getDuplicateFrom_get() {
		...
		$duplicateData = $this->documentduplicate->getDuplicate('dvnd','vnd',$Data['dms_card_code'],$Data['business']);
