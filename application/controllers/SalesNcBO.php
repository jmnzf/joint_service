...
class SalesNcBO extends REST_Controller
{
	...
	public function createSalesNc_post()
	{
		...
		//
		$FactorC = true; // Impuesto con factor de conversion
		...
		// VARIFICAR FECHA DE VENCIMIENTO
		if($Data['vnc_duedate'] < $Data['vnc_docdate']){
			$respuesta = array(
				'error' => true,
				'data' => [],
				'mensaje' => 'La fecha de vencimiento ('.$Data['vnc_duedate'].') no puede ser inferior a la fecha del documento ('.$Data['vnc_docdate'].')'
			);
			...
			if (is_numeric($resInsert) && $resInsert > 0) {
				...
				foreach ($ContenidoDetalle as $key => $detail) {
					...
					$sqlInsertDetail = "INSERT INTO vnc1(nc1_docentry, nc1_itemcode, nc1_itemname, nc1_quantity, nc1_uom, nc1_whscode,
										...
										nc1_ubication,nc1_baseline,ote_code,detalle_modular,nc1_tax_base,detalle_anuncio,imponible, nc1_codmunicipality)VALUES(:nc1_docentry, :nc1_itemcode, :nc1_itemname, :nc1_quantity,:nc1_uom, :nc1_whscode,:nc1_price, 
										...
										:nc1_ubication,:nc1_baseline,:ote_code,:detalle_modular,:nc1_tax_base,:detalle_anuncio,:imponible, :nc1_codmunicipality)";
								...
								foreach ($ContenidoRentencion as $key => $value) {
									...
									if (is_numeric($resInsertRetenciones) && $resInsertRetenciones > 0) {
										...
										$respuesta = array(
											...
											'mensaje'	=> 'No se pudo registrar la nota credito de ventas, fallo el proceso para insertar las retenciones'
										);
					...
					if ($exc_inv == 1) {
						...
						if ($ManejaInvetario == 1) {
							...
							if (isset($resItemSerial[0])) {
								...
								$AddSerial = $this->generic->addSerial($detail['serials'], $detail['nc1_itemcode'], $Data['vnc_doctype'], $resInsert, $DocNumVerificado, $Data['vnc_docdate'], 1, $Data['vnc_comment'], $detail['nc1_whscode'], $detail['nc1_quantity'], $Data['vnc_createby'], $resInsertDetail, $Data['business']);
					...
					if ($FactorC) {
						...
						if ( isset( $detail['nc1_discount'] ) &&  isset( $detail['nc1_discount'] ) && $detail['nc1_discount'] > 0 ) {
							...
							$DetalleAsientoIngreso->descuento = $DetalleAsientoDescuento->descuento;
							...
							$DetalleAsientoIngreso->descuento = 0;
						}
				...
				foreach ($DetalleConsolidadoIngreso as $key => $posicion) {
					...
					foreach ($posicion as $key => $value) {
						...
						if (  $value->gift == 0 ) {
							...
							$granTotalIngreso = ($granTotalIngreso + ( ($value->nc1_price * $value->nc1_quantity) - $value->nc1_vatsum) );
							$granTotalTasaFija = ($granTotalTasaFija + ( ($value->nc1_price * $value->nc1_quantity) - $value->nc1_vatsum) );

							$granTotalIngreso = ($granTotalIngreso + $value->descuento);
							$granTotalTasaFija = ($granTotalTasaFija + $value->descuento);
							...
							if ( isset($CUENTASINV['error']) && $CUENTASINV['error'] == false ) {
								$cuenta = $CUENTASINV['data']['acct_return'];
							}else{
								$this->pedeo->trans_rollback();
								...
								$respuesta = array(
									...
									'mensaje'	=> 'No se encontro la cuenta de devolución para el item ' . $value->nc1_itemcode
								);
					...
					if ( $debito > 0 ||  $credito > 0 ) {
						...
						if ( $debito > 0 ){
							$BALANCE = $this->account->addBalance($periodo['data'], round($debito, $DECI_MALES), $cuenta, 1, $Data['vnc_docdate'], $Data['business'], $Data['branch']);
						}else{
							$BALANCE = $this->account->addBalance($periodo['data'], round($credito, $DECI_MALES), $cuenta, 2, $Data['vnc_docdate'], $Data['business'], $Data['branch']);
						}
				...
				foreach ($DetalleConsolidadoIva as $key => $posicion) {
					...
					foreach ($posicion as $key => $value) {
						...
						$granTotalIva2 = $granTotalIva;

						
						$CodigoImp = $value->codimp;// CODIGO DE IMPUESTO
						$Vat = $value->nc1_vat; // TASA DE IMPUESTO
					...
					$BALANCE = $this->account->addBalance($periodo['data'], round($granTotalIva, $DECI_MALES), $value->nc1_cuentaIva, 1, $Data['vnc_docdate'], $Data['business'], $Data['branch']);
					if (isset($BALANCE['error']) && $BALANCE['error'] == true){
				...
				foreach ($DetalleConsolidadoCostoInventario as $key => $posicion) {
					...
					// SE AGREGA AL BALANCE
					if ( $dbito > 0 ){
						$BALANCE = $this->account->addBalance($periodo['data'], round($dbito, $DECI_MALES), $cuentaInventario, 1, $Data['vnc_docdate'], $Data['business'], $Data['branch']);
					}else{
						$BALANCE = $this->account->addBalance($periodo['data'], round($cdito, $DECI_MALES), $cuentaInventario, 2, $Data['vnc_docdate'], $Data['business'], $Data['branch']);
					}
					if (isset($BALANCE['error']) && $BALANCE['error'] == true){
				...
				foreach ($DetalleConsolidadoCostoCosto as $key => $posicion) {
					...
					if ( $dbito > 0 || $cdito > 0 ){
						...
						// SE AGREGA AL BALANCE
						if ( $dbito > 0 ){
							$BALANCE = $this->account->addBalance($periodo['data'], round($dbito, $DECI_MALES), $cuentaCosto, 1, $Data['vnc_docdate'], $Data['business'], $Data['branch']);
						}else{
							$BALANCE = $this->account->addBalance($periodo['data'], round($cdito, $DECI_MALES), $cuentaCosto, 2, $Data['vnc_docdate'], $Data['business'], $Data['branch']);
						}
						if (isset($BALANCE['error']) && $BALANCE['error'] == true){
				...
				if (isset($rescuentaCxC[0])) {
					...
					$MontoSysDB = 0;
					$MontoSysCR = 0;
					$docTotal = $Data['vnc_doctotal'];
					$docTotalOriginal = $docTotal;
					$docTotalSys = 0;
					$MontoIGTF = 0;
					$MontoIGTFSYS = 0;

					if (isset($Data['vnc_igtf']) && $Data['vnc_igtf']  > 0) {

						$MontoIGTF = $Data['vnc_igtf'];
						$MontoIGTFSYS  = $Data['vnc_igtf'];
					...
					if (trim($Data['vnc_currency']) != $MONEDASYS) {
						$docTotalSys = ($docTotal / $TasaLocSys);
					} else {
						$docTotalSys = $docTotalOriginal;
					}
					...
					if ( $debitoo > 0 || $creditoo > 0 ){
						...
						// SE AGREGA AL BALANCE
						if ( $debitoo > 0 ){
							$BALANCE = $this->account->addBalance($periodo['data'], round($debitoo, $DECI_MALES), $cuentaCxC, 1, $Data['vnc_docdate'], $Data['business'], $Data['branch']);
						}else{
							$BALANCE = $this->account->addBalance($periodo['data'], round($creditoo, $DECI_MALES), $cuentaCxC, 2, $Data['vnc_docdate'], $Data['business'], $Data['branch']);
						}
						if (isset($BALANCE['error']) && $BALANCE['error'] == true){
				...
				if ($FactorC) {
					...
					if (isset($DetalleConsolidadoDescuento[0])){
						...
						$cuentaDescuento = "SELECT coalesce(pge_sales_discount_account, 0) as cuenta FROM pgem WHERE pge_id = :pge_id";
						...
						if (isset( $rescuentaDescuento[0] ) && $rescuentaDescuento[0]['cuenta'] > 0 ) {
							...
							$respuesta = array(
								...
								'mensaje'	=> 'No se pudo registrar la nota credito de ventas, no se encontro la cuenta para el descuento'
							);
						...
						$BALANCE = $this->account->addBalance($periodo['data'], round($totalDescuento, $DECI_MALES), $cuentadescuento, 2, $Data['vnc_docdate'], $Data['business'], $Data['branch']);
						if (isset($BALANCE['error']) && $BALANCE['error'] == true){
					...
					if (isset($DetalleConsolidadoIvaDescuento[0])){
						...
						if (isset( $rescuentaIvaDescuento[0] ) && $rescuentaIvaDescuento[0]['cuenta'] > 0 ) {
							...
							$respuesta = array(
								...
								'mensaje'	=> 'No se pudo registrar la nota credito de ventas, no se encontro la cuenta para asociar el iva del descuento'
							);
						...
						$BALANCE = $this->account->addBalance($periodo['data'], round($totalIvaDescuento, $DECI_MALES), $cuentaivadescuento, 2, $Data['vnc_docdate'], $Data['business'], $Data['branch']);
						if (isset($BALANCE['error']) && $BALANCE['error'] == true){
				...
				if (isset($Data['vnc_igtf']) && $Data['vnc_igtf'] > 0) {
					...
					if (isset($resCuentaIGTF[0]) && $resCuentaIGTF[0]['imm_acctcode'] > 0) {
						...
						if (trim($Data['vnc_currency']) != $MONEDASYS) {
							...
							$MontoSysDB = $Data['vnc_igtf'];
						}

						// SE AGREGA AL BALANCE
						if ( $dbito > 0 ){
							$BALANCE = $this->account->addBalance($periodo['data'], round($dbito, $DECI_MALES), $resCuentaIGTF[0]['imm_acctcode'], 1, $Data['vnc_docdate'], $Data['business'], $Data['branch']);
						}else{
							$BALANCE = $this->account->addBalance($periodo['data'], round($cdito, $DECI_MALES), $resCuentaIGTF[0]['imm_acctcode'], 2, $Data['vnc_docdate'], $Data['business'], $Data['branch']);
						}
						if (isset($BALANCE['error']) && $BALANCE['error'] == true){
				...
				foreach ($DetalleConsolidadoRetencion as $key => $posicion) {
					...
					$BALANCE = $this->account->addBalance($periodo['data'], round($totalRetencion, $DECI_MALES), $cuenta, 2, $Data['vnc_docdate'], $Data['business'], $Data['branch']);
					if (isset($BALANCE['error']) && $BALANCE['error'] == true){
				...
				if ($Data['vnc_basetype'] == 5) { // SOLO CUANDO ES UNA FACTURA
					...
					if (is_numeric($resUpdateFactPay) && $resUpdateFactPay == 1) {
						...
						$valorF = $this->pedeo->queryTable("SELECT * FROM dvfv WHERE dvf_docentry = :dvf_docentry AND dvf_doctype = :dvf_doctype", array(
							':dvf_docentry' => $Data['vnc_baseentry'],
							':dvf_doctype'  => $Data['vnc_basetype']
						));
						...
						if ( isset($valorF[0]) ) {
							...
							if ( $valorF[0]['dvf_paytoday'] > $valorF[0]['dvf_doctotal'] ) {
								...
								$respuesta = array(
									...
									'mensaje'	=> 'El monto aplicado supera el monto total de la factura ID # '.$Data['vnc_baseentry']
								);
							...
							$respuesta = array(
								'error'   => true,
								'data' => $resUpdateFactPay,
								'mensaje'	=> 'No se pudo verificar el valor aplicado por la nota credito a la factura ID # '.$Data['vnc_baseentry']
							);
						...
						$respuesta = array(
							'error'   => true,
							'data' => $resUpdateFactPay,
							'mensaje'	=> 'No se pudo actualizar el valor del pago en la factura (paytoday)' . $Data['vnc_baseentry']
						);
				...
				if ($Data['vnc_basetype'] == 5) { // SOLO CUANDO ES UNA FACTURA
					...
					// VERIFICAR FACTURA DE SOLO OBSEQUIO
					$fvob = "SELECT coalesce(dvf_doctotal,0) as total FROM dvfv WHERE dvf_docentry = :dvf_docentry and dvf_doctype = :dvf_doctype";
					$resFvob = $this->pedeo->queryTable($fvob, array(
						':dvf_docentry' => $Data['vnc_baseentry'],
						':dvf_doctype'  => $Data['vnc_basetype']
					));

					if ( $resFvob[0]['total'] > 0 ){
						...
						if(isset($resDetailFV[0])){
							...
							if (!isset($rescuentaCxC[0])) {
								...
								$respuesta = array(
									'error'   => true,
									'data'	  => $rescuentaCxC,
									'mensaje'	=> 'No se pudo registrar la nota credito de ventas, el tercero no tiene cuenta asociada'
								);
							...
							if (is_numeric($resUpdateVenDebit) && $resUpdateVenDebit == 1) {
								...
								$respuesta = array(
									...
									'mensaje'	=> 'No se pudo actualizar el valor del pago en la factura (vencredit)' . $Data['vnc_baseentry']
								);
				...
				//SE VALIDA LA CONTABILIDAD CREADA
				$validateCont2 = $this->generic->validateAccountingAccent($resInsertAsiento);
				...
				$respuesta = array(
					'error' => false,
					'data' => $resInsert,
					'mensaje' => 'Nota crédito de clientes #'.$DocNumVerificado.' registrada con exito'
				);
			} else {
	...
	public function updateSalesNc_post()
	{
		...
		$sqlUpdate = "UPDATE dvnc	SET vnc_docdate=:vnc_docdate,vnc_duedate=:vnc_duedate, vnc_duedev=:vnc_duedev, vnc_pricelist=:vnc_pricelist, vnc_cardcode=:vnc_cardcode,
				...
				vnc_adress=:vnc_adress, vnc_paytype=:vnc_paytype , vnc_internal_comments = :vnc_internal_comments WHERE vnc_docentry=:vnc_docentry";
		...
		if (is_numeric($resUpdate) && $resUpdate == 1) {
			...
			foreach ($ContenidoDetalle as $key => $detail) {
				...
				$sqlInsertDetail = "INSERT INTO vnc1(nc1_docentry, nc1_itemcode, nc1_itemname, nc1_quantity, nc1_uom, nc1_whscode,
																			nc1_price, nc1_vat, nc1_vatsum, nc1_discount, nc1_linetotal, nc1_costcode, nc1_ubusiness, nc1_project,
																			nc1_acctcode, nc1_basetype, nc1_doctype, nc1_avprice, nc1_inventory,nc1_cuentaIva,nc1_ubication,ote_code, nc1_codmunicipality)VALUES(:nc1_docentry, :nc1_itemcode, :nc1_itemname, :nc1_quantity,
																			:nc1_uom, :nc1_whscode,:nc1_price, :nc1_vat, :nc1_vatsum, :nc1_discount, :nc1_linetotal, :nc1_costcode, :nc1_ubusiness, :nc1_project,
																			:nc1_acctcode, :nc1_basetype, :nc1_doctype, :nc1_avprice, :nc1_inventory,:nc1_cuentaIva,:nc1_ubication,:ote_code, :nc1_codmunicipality)";
	...
	public function getDuplicateFrom_get() {
		...
		$duplicateData = $this->documentduplicate->getDuplicate('dvnc','vnc',$Data['dms_card_code'],$Data['business']);
	...
	public function getDuplicateDt_get()
	{
		...
		if (!isset($Data['nc1_docentry'])) {
			...
			$copy = $this->documentduplicate->getDuplicateDt($Data['nc1_docentry'],'dvnc','vnc1','vnc','nc1','detalle_modular::jsonb,imponible');
