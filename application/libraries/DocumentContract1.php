...
class DocumentContract1 {
	...
	public function format($Data,$type_mpdf = "D"){
		...
		FROM pgem WHERE pge_id = :pge_id", array(':pge_id' => $Data['business']));
			...
			$contenidoContrato = $this->ci->pedeo->queryTable($sqlcotizacion,array(':csn_docentry'=>$Data['docentry']));
				...
				$resSqlDetalle2 = $this->ci->pedeo->queryTable($sqlDetalle2,array(':sn2_docentry' => $value['entry']));
				foreach ($resSqlDetalle2 as $key => $value1) {
					if($value1['itementrega'] == 5 && $value['referencia'] == $value1['item']){
						switch ($value1['day']){
							case '1':
					...
					}else if ($value1['itementrega'] == 6 && $value['referencia'] == $value1['item']){
        ...
        if($type_mpdf == "D"){
			...
			return array(
				...
				'data' => array(
					'docnum' => $consecutivo,
					'cardname' => $contenidoContrato[0]['cliente'],
					'total' => $contenidoContrato[0]['monedadocumento']." ".number_format($contenidoContrato[0]['totaldoc'], $DECI_MALES, ',', '.'),
					'type' => $Type
				)
			);
		}
