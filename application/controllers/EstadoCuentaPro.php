	...
	public function EstadoCuentaPro_post(){
	          ...
	          return;
				...
				$sqlestadocuenta = "SELECT
                        	...
                        	'".$Data['fecha']."' FechaCorte,
													('".$Data['fecha']."' - t0.cfc_duedate) dias,
                        	CASE
                        		WHEN ( '".$Data['fecha']."' - t0.cfc_duedate) >=0 and ( '".$Data['fecha']."' - t0.cfc_duedate) <=30
                        			then (t0.cfc_doctotal - COALESCE(t0.cfc_paytoday,0))
															ELSE 0
                        	END uno_treinta,
                        	CASE
                        		WHEN ( '".$Data['fecha']."' - t0.cfc_duedate) >=31 and ( '".$Data['fecha']."' - t0.cfc_duedate) <=60
                        			then (t0.cfc_doctotal - COALESCE(t0.cfc_paytoday,0))
															ELSE 0
                        	END treinta_uno_secenta,
                        	CASE
                        		WHEN ( '".$Data['fecha']."' - t0.cfc_duedate) >=61 and ( '".$Data['fecha']."' - t0.cfc_duedate) <=90
                        			then (t0.cfc_doctotal - COALESCE(t0.cfc_paytoday,0))
															ELSE 0
                        	END secenta_uno_noventa,
                        	CASE
                        		WHEN ( '".$Data['fecha']."' - t0.cfc_duedate) >=91
                        			then (t0.cfc_doctotal - COALESCE(t0.cfc_paytoday,0))
													ELSE 0
                        	END mayor_noventa

                        FROM dcfc t0
                        WHERE '".$Data['fecha']."' >= t0.cfc_duedate  and t0.cfc_cardcode = '".$Data['cardcode']."'
        ...
        <tr>
            ...
            <th style="text-align: center; margin-left: 600px;">
                <p>INFORME ESTADO DE CUENTA PROVEEDORES</p>
