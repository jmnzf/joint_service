...
class PdfSolTranfStock extends REST_Controller {
	...
	public function PdfSolTranfStock_post(){
		...
		FROM pgem WHERE pge_id = :pge_id", array(':pge_id' => $Data['business']));
	          ...
	          return;
				...
				$sqlEntradaInv = "SELECT distinct
                                    CONCAT(T0.ist_CARDNAME,' ',T2.DMS_CARD_LAST_NAME) Cliente,
                                    TRIM('CN' FROM T0.ist_CARDCODE) Nit,
                                    CONCAT(T3.DMD_ADRESS,' ',T3.DMD_CITY) Direccion,
                                    T4.DMC_PHONE1 Telefono,
                                    T4.DMC_EMAIL Email,
                                    CONCAT(T6.PGS_PREF_NUM,' ',T0.ist_DOCNUM) NumeroDocumento,
                                    T0.ist_DOCDATE FechaDocumento,
                                    coalesce(T0.ist_DUEDATE,current_date) FechaVenDocumento,
                                    trim('COP' FROM CAST(t0.ist_CURRENCY AS VARCHAR)) MonedaDocumento,
                                    T7.PGM_NAME_MONEDA NOMBREMONEDA,
                                    T5.MEV_NAMES Vendedor,
                                    '' MedioPago,
                                    '' CondPago,
                                    T1.st1_ITEMCODE Referencia,
                                    T1.st1_ITEMNAME descripcion,
                                    T1.st1_WHSCODE Almacen,
                                    T1.st1_UOM UM,
                                    T1.st1_QUANTITY Cantidad,
                                    T1.st1_PRICE VrUnit,
                                    T1.st1_DISCOUNT PrcDes,
                                    T1.st1_VATSUM IVAP,
                                    T1.st1_LINETOTAL ValorTotalL,
                                    T0.ist_BASEAMNT base,
                                    T0.ist_DISCOUNT Descuento,
                                    (T0.ist_BASEAMNT - T0.ist_DISCOUNT) subtotal,
                                    T0.ist_TAXTOTAL Iva,
                                    T0.ist_DOCTOTAL TotalDoc,
                                    T0.ist_COMMENT Comentarios
                                FROM dist t0
                                INNER JOIN ist1 T1 ON t0.ist_docentry = t1.st1_docentry
                                LEFT JOIN DMSN T2 ON t0.ist_cardcode = t2.dms_card_code
                                LEFT JOIN DMSD T3 ON T0.ist_ADRESS = CAST(T3.DMD_ID AS VARCHAR) AND t3.dmd_ppal = 1
                                LEFT JOIN DMSC T4 ON T0.ist_CONTACID = CAST(T4.DMC_ID AS VARCHAR)
                                LEFT JOIN DMEV T5 ON T0.ist_SLPCODE = T5.MEV_ID
                                LEFT JOIN PGDN T6 ON T0.ist_DOCTYPE = T6.PGS_ID_DOC_TYPE AND T0.ist_SERIES = T6.PGS_ID
                                LEFT JOIN PGEC T7 ON CAST(T0.ist_CURRENCY AS VARCHAR) = T7.PGM_SYMBOL
                                WHERE T0.ist_DOCENTRY = :ist_DOCENTRY and t0.business = :business";

				$ContenidoEntradaInv = $this->pedeo->queryTable($sqlEntradaInv,array(
					':IST_DOCENTRY'=>$Data['IST_DOCENTRY'],
				...
				$resSerial = $this->pedeo->queryTable($sqlSerial, array(
					':business' 	 => $Data['business'],
					':msn_basetype'  => 9,
					':msn_baseentry' => $Data['IST_DOCENTRY']
				));
