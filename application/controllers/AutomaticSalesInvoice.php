...
class AutomaticSalesInvoice extends REST_Controller
{
	...
	public function createAutomaticSalesInvoice_post()
	{


        $sqlHeader = "SELECT tcsn.*,
        			case 
        ...
        $sqlDetail = "SELECT * FROM csn1 WHERE sn1_docentry = :sn1_docentry AND sn1_enabled = :sn1_enabled";



        $resHeader = $this->pedeo->queryTable($sqlHeader, array());


        if (!isset($resHeader[0])) {
        ...
        // ARMANDO EL ARRAY DE DIAS PARA FACTURAR Y HACIENDO EL FILTRO POR

        // Formatear la fecha para obtener el nombre del mes en español
        $nombreMes = self::NombreMesES(date('m'));
				...
				while (!$valido) {
					...
					if ( self::esFechaValida($fecha) ) {
                        ...
                        if ( $fecha == $fechaActual ) {
                            $resHeader[$key]['corte']  = ' CORTE '.$nombreMes.'-'.date("Y");
                            array_push($dataValida, $resHeader[$key]);
                        }
        ...
        //
        $Data = [];
        $resHeader = $dataValida;

        foreach ( $resHeader as $key => $element ) {

            $fechaActual = date('Y-m-d'); // Obtiene la fecha actual en formato YYYY-MM-DD
            $nuevaFecha = date('Y-m-d', strtotime($fechaActual . ' + 7 days'));
            ...
            // CAMBIAR DIRECCION DE FACTURA PARA DIRECCION PRINCIPAL DEL CLIENTE
            $idDir = 0;
            $adDres = "";
            $resDir = $this->pedeo->queryTable("SELECT dmd_id, dmd_adress FROM dmsd WHERE  dmd_card_code  = :dmd_card_code AND dmd_ppal = :dmd_ppal", array(
                ":dmd_card_code" => $element['csn_cardcode'],
                ':dmd_ppal'      => 1
            ));

            if ( isset($resDir[0]) ) {
                $idDir = $resDir[0]['dmd_id'];
            ...
            $Data = array(
                'dvf_docnum' => 0,
				'dvf_series' => 2,
				'dvf_docdate' => date('Y-m-d'),
				'dvf_duedate' => $vencimiento,
				'dvf_duedev' => date('Y-m-d'),
				'dvf_pricelist' => $element['csn_pricelist'],
				'dvf_cardcode' => $element['csn_cardcode'],
				'dvf_cardname' => $element['csn_cardname'],
				'dvf_currency' => $element['csn_currency'],
				'dvf_contacid' => $element['csn_contacid'],
				'dvf_slpcode' => $element['csn_slpcode'],
				'dvf_empid' => $element['csn_empid'],
				'dvf_comment' => $element['csn_comment'].' '.$element['corte'],
				'dvf_doctotal' => $element['csn_doctotal'],
				'dvf_baseamnt' => $element['csn_baseamnt'],
				'dvf_taxtotal' => $element['csn_taxtotal'],
				'dvf_discprofit' => $element['csn_discprofit'],
				'dvf_discount' => $element['csn_discount'],
				'dvf_createat' =>  date('Y-m-d'),
				'dvf_baseentry' => $element['csn_docentry'],
				'dvf_basetype' => $element['csn_doctype'],
				'dvf_doctype' => 5,
				'dvf_idadd' => $idDir,
				'dvf_adress' => $adDres,
				'dvf_paytype' => $element['csn_paytype'],
				'dvf_createby' => 'manager',
				'dvf_correl' => NULL,
				'dvf_transport' => NULL,
				'dvf_sub_transport' => NULL,
				'dvf_ci' => NULL,
				'dvf_t_vehiculo' => NULL,
				'dvf_guia' =>  NULL,
				'dvf_placa' => NULL,
				...
				'dvf_totalretiva' =>  0,
				'dvf_bankable' => 0,
				'dvf_internal_comments' => $element['csn_comment'].' '.$element['corte'],
            );


            $resDetail = $this->pedeo->queryTable($sqlDetail, array(":sn1_docentry" => $element['csn_docentry'], ":sn1_enabled" => 1));


            if (isset($resDetail[0])) {
                ...
                foreach ($resDetail as $key => $value) {
                    ...
                    array_push($Data['detail'], array(
                        ...
                        'fv1_itemcode' => $value['sn1_itemcode'], 
                        'fv1_itemname' => $value['sn1_itemname'] ,
                        'fv1_quantity' => $value['sn1_quantity'] ,
                        'fv1_uom' => $value['sn1_uom'], 
                        'fv1_whscode' => $value['sn1_whscode'] ,
                        'fv1_price' => $value['sn1_price'] ,
                        'fv1_vat' => $value['sn1_vat'] ,
                        'fv1_vatsum' => $value['sn1_vatsum'] ,
                        'fv1_discount' => $value['sn1_discount'] ,
                        'fv1_linetotal' => $value['sn1_linetotal'] ,
                        'fv1_costcode' => $value['sn1_costcode'] ,
                        'fv1_ubusiness' => $value['sn1_ubusiness'] ,
                        'fv1_project' => $value['sn1_project'] ,
                        'fv1_acctcode' => $value['sn1_acctcode'] ,
                        'fv1_basetype' => $value['sn1_basetype'] ,
                        'fv1_doctype' => $value['sn1_doctype'] ,
                        'fv1_avprice' => $value['sn1_avprice'] ,
                        'fv1_inventory' => $value['sn1_inventory'] ,
                        'fv1_cuentaIva'  => $value['sn1_acciva'] ,
                        'fv1_fixrate' => 0 ,
                        'fv1_codimp' => $value['sn1_codimp'] ,
                        'fv1_ubication' => $value['sn1_ubication'] ,
                        'fv1_linenum' => $value['sn1_linenum']  ,
                        'fv1_baseline' => $value['sn1_baseline']  ,
                        'ote_code' => $value['ote_code'] ,
                        'fv1_gift' => 0 ,
                        'detalle_modular' => NULL , 
                        'fv1_tax_base' => $value['sn1_tax_base']  ,
                        'detalle_anuncio' => NULL ,
                        'imponible' => 0,
                        'detail' => isset($resRetencion[0]) ? $resRetencion : []

                    ));

                    $totaDoc = $totaDoc + ( $value['sn1_linetotal'] + $value['sn1_vatsum'] );
                ...
                // LLENANDO DATOS DE CABECERA ACTUALIZADOS
                    $Data['dvf_doctotal']   = ( ( $totaDoc - $totalReteFuente ) - $totalReteIva ) ;
				    $Data['dvf_baseamnt']   = $subTotal;
				    $Data['dvf_taxtotal']   = $totalImpuesto;
				    $Data['dvf_discprofit'] = $totalDesc;
				    $Data['dvf_discount']   = $totalDesc;

                    $Data['dvf_totalret']    =  $totalReteFuente > 0 ? ($totalReteFuente * -1) : 0;
				    $Data['dvf_totalretiva'] =  $totalReteIva > 0 ? ($totalReteIva * -1) : 0;
                //
                ...
                $sqlDetalleAsiento = "INSERT INTO mac1(ac1_trans_id, ac1_account, ac1_debit, ac1_credit, ac1_debit_sys, ac1_credit_sys, ac1_currex, ac1_doc_date, ac1_doc_duedate,
                            ...
                            ac1_ven_debit,ac1_ven_credit, ac1_fiscal_acct, ac1_taxid, ac1_isrti, ac1_basert, ac1_mmcode, ac1_legal_num, ac1_codref, ac1_line, ac1_base_tax, business, 
                            branch, ac1_codret) VALUES (:ac1_trans_id, :ac1_account,
    ...
    // VALIDA SI UNA FECHA ESTA EN EL CALENDARIO
	private static function NombreMesES($mes) {
