...
class AccountReconciliations extends REST_Controller
{
    ...
    public function applyAccountReconciliations_post()
    {
        ...
        $DocNumVerificado = $this->documentnumbering->NumberDoc($Data['crc_series'], $Data['crc_docdate'], $Data['crc_docdate']);
		...
		$s2 = "";
		$ct2 = "";
		// VALIDANDO DATOS DE LOS SOCIOS
		if ( is_array( $Data['crc_cardcode'] ) && is_array( $Data['crc_cardtype'] ) ) {

			if ( isset( $Data['crc_cardcode'][2] ) ){
			...
			for ($i=0; $i < count($Data['crc_cardtype']); $i++) { 

				if ($i == 0){
					$s1 = $Data['crc_cardcode'][$i];
					...
					if ($detail['rc1_doctype'] == 34) {
                        ...
                        if (is_numeric($resUpdateFactPay) && $resUpdateFactPay == 1) {
                            ...
                            $respuesta = array(
                                'error' => true,
                                'data' => $resUpdateFactPay,
                                'mensaje' => 'No se pudo actualizar el valor del pago en la factura anticipada ' . $detail['rc1_docentry'],
                            );
