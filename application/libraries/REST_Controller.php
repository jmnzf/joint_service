...
abstract class REST_Controller extends \CI_Controller {
     ...
     */
    public function getColumn($table,$prefijo,$campos='',$inner ='', $decimals = 2, $business, $branch, $doctype = 0, $page = 0, $paginated = 0,$where = "",$cardtype = 1) {
        ...
        if ( $doctype == 0 ){
            ...
            $doctype = " AND t0.".$prefijo."_"."doctype =".$doctype;
        }
        ...
        // SE VERIFICA SI SE REQUIERE PAGINAR EL RESULTADO POR DEFECTO 10 LINEAS DEVUELTAS
        $LIMIT   = 10;
        $OFFSET  = 0;
        $LIMITOFF = "";
        ...
        $query = "SELECT DISTINCT
            ...
            t0.{prefix}_cardcode,
          	t0.{prefix}_cardname,
          	t0.{prefix}_comment,
            t0.{prefix}_internal_comments,
            t0.{prefix}_createby,
            t0.{prefix}_origen AS origin,
            t0.{prefix}_currency,
            t0.{prefix}_basetype,
            t0.{prefix}_baseentry,
            t0.{prefix}_taxtotal_ad,
            t7.dmd_adress direccion,
            ...
            t0.{prefix}_doctotal as {prefix}_doctotal1,
            ...
            CONCAT(T0.{prefix}_CURRENCY,' ',TRIM(TO_CHAR({prefix}_taxtotal_ad,'{format}'))) imp_ad,
            ...
            LEFT JOIN DMSN T4 ON t0.{prefix}_cardcode = t4.dms_card_code {cardtype}
        ...
        // EMPLEADO DE VENTAS
        $query = str_replace("{dmev}", ($type == 37 ? "LEFT JOIN dmev t2 on t0.{prefix}_slpcode = t2.mev_id" : "LEFT JOIN dmev t2 on t0.{prefix}_slpcode = t2.mev_id"), $query);
        ...
        $query = str_replace("{format}", "999,999,999,999.".$decimals, $query);
        // REEMPLAZAR INNER ADICIONALES.
        $query = str_replace("{innerjoin}", $inner, $query);
        // EMPRESA
        $query = str_replace("{business}", $business, $query);
        // SUCURSAL
        $query = str_replace("{branch}", $branch, $query);
        // TIPO DOCUMENTO PARA DIFERENCIAR FACTURA ANTICIPADA
        $query = str_replace("{doctype}", $doctype, $query);
        // PAGINACION DE RESULTADOS
        $query = str_replace("{limitoff}", $LIMITOFF, $query);
        //ADICIONAR WHERE
        $query = str_replace("{where}",$where,$query);
        //VALIDAR EL TIPO DEL SOCIO
        if($cardtype == 1){
            $query = str_replace("{cardtype}"," and t4.dms_card_type = '1'",$query);
        }else if ($cardtype == 2){
            $query = str_replace("{cardtype}"," and t4.dms_card_type = '2'",$query);
        }
        // RETORNAR QUERY.
        // print_r($query);exit;
        return $query;
